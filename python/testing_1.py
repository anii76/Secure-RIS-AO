...
#H_AU = np.array([Rician_Fading_Channels(M, 1, dist_AU, pl, zeta, K)
#                for _ in range(N_samples)])
#H_AE = np.array([Rician_Fading_Channels(M, 1, dist_AE, pl, zeta, K)
#                for _ in range(N_samples)])
H_AI = np.array([Rician_Fading_Channels(M, N, dist_AI, pl_AI, zeta, K)
                for _ in range(N_samples)])
H_IU = np.array([Rician_Fading_Channels(N, n, dist_IU, pl, zeta, K)
                for _ in range(N_samples)])
H_IE = np.array([Rician_Fading_Channels(N, n, dist_IE, pl, zeta, K)
                for _ in range(N_samples)])
